...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var listRecipeSelect: RecipeResponse? //para poder seleccionar una celda
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        listRecipeSelect = arrayRecipe[indexPath.row] //a la celda seleccionada le pasamos el array de RecipeResponse
        let storyBoardMain = UIStoryboard(name: "Main", bundle: nil) //llamamos al storyboard Main
        let viewController = storyBoardMain.instantiateViewController(withIdentifier: "DetailViewController") as? DetailViewController //al storyboard le agregamos el destino DetailViewController
        viewController?.recipeDetail = listRecipeSelect //al detalle del destino le pasamos lo que seleccionamos
        self.navigationController?.pushViewController(viewController ?? ViewController(), animated: true) //navegamos a la pantalla siguiente
    }
}
